...
def main(args):
    *_, func_args = inspect.getargvalues(inspect.currentframe())
    ...
    for model_idx, (config_key, model_config) in enumerate(list(config.items())):
        ...
        print (f"Controlnet Video : {model_config.controlnet_video}, face : {model_config.include_face}, hand: {model_config.include_hand}")
        if model_config.controlnet:
            if model_config.controlnet_video.endswith(".mp4"):
                vr = decord.VideoReader(model_config.controlnet_video, width=512, height=512)
    
                video = vr.get_batch(list(range(16)))
                poses = [pose_model(v, include_face=model_config.include_face, include_hand = model_config.include_hand) for v in video]
            else:#assuming single photo
                poses = [pose_model(PIL.Image.open(model_config.controlnet_video), include_hand = model_config.include_hand)] * 16
            #poses[0].save("pose0.png")
            ind = 0
            for pos in poses:
                pos.save(f"poses{ind}.png")
                ind += 1
        ...
        for motion_module in motion_modules:
            ...
            pipeline = AnimationPipeline(
                ...
                controlnet = controlnet if model_config.controlnet else None
            ).to("cuda")
            ...
            for prompt_idx, (prompt, n_prompt, random_seed) in enumerate(zip(prompts, n_prompts, random_seeds)):
                ...
                sample = pipeline(
                    ...
                    fp16                = not args.fp32,
                    image = poses if model_config.controlnet else None
                ).videos
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pretrained_model_path", type=str, default="/home/avcr/Desktop/cem/animatediff_finetune/AnimateDiff/models/StableDiffusion")
